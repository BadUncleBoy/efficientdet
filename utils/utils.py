...
def postprocess(x, anchors, regression, classification, regressBoxes, clipBoxes, threshold, iou_threshold, anchor_free_mode=False):
    out = []
    if anchor_free_mode:
        img_size = x.shape[2]
        ...
        Boxes = regression * float(img_size)
        scores, classes = torch.max(classification, dim=2)

        for i in range(x.shape[0]):
            scores_per = scores[i]
            classes_per = classes[i]
            bbox_per    = Boxes[i]
            scores_over_thresh = scores_per > threshold
            scores_per = scores_per[scores_over_thresh]
            classes_per = classes_per[scores_over_thresh]
            bbox_per    = bbox_per[scores_over_thresh]
            if scores_over_thresh.sum() == 0:
                ...
                continue
            keep_idxs = batched_nms(bbox_per, scores_per, classes_per, iou_threshold=iou_threshold)
            if keep_idxs.shape[0] != 0:
                out.append({
                    'rois': bbox_per[keep_idxs].cpu().numpy(),
                    'class_ids': classes_per[keep_idxs].cpu().numpy(),
                    'scores': scores_per[keep_idxs].cpu().numpy(),
                })
            else:
                out.append({
                    'rois': np.array(()),
...
def display(preds, imgs, obj_list, imshow=True, imwrite=False):
    for i in range(len(imgs)):
        ...
        if imwrite:
            ...
            cv2.imwrite('test/{0}.jpg'.format(uuid.uuid4().hex), imgs[i])
...
class CustomDataParallel(nn.DataParallel):
    ...
    def scatter(self, inputs, kwargs, device_ids):
        ...
        return [(inputs[0][splits * device_idx: splits * (device_idx + 1)].to('cuda:{0}'.format(device_idx), non_blocking=True),
                 inputs[1][splits * device_idx: splits * (device_idx + 1)].to('cuda:{0}'.format(device_idx), non_blocking=True))
...
def get_last_weights(weights_path):
    ...
    weights_path = sorted(weights_path,
                          ...
                          reverse=True)[0]
    print('using weights {0}'.format(weights_path))
    return weights_path
...
def parse_gt_rec(gt_filename):
    ...
    with open(gt_filename, 'r') as f:
        for line in f:
            ...
            gt_dict[line_idx] = boxes
    return gt_dict, img_pathes
